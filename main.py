...
from realtoxicityprompts import ToxicityDetectionLayer
...
# Toxicity
toxicity_filter = ToxicityDetectionLayer("./custom_classifier")
...
async def streaming_response(websocket: WebSocket, Bot, user_id, prompt, **bot_args):  #(websocket, MainAssistant, user_id, user_info=user_info, query_results=res)
    '''
    ...
    async def bot_respond(**kwargs):
        ...
        get_logprobs = kwargs["get_logprobs"]
        bot_args = kwargs.get("bot_args", {})
        return await Bot.respond(messages=messages, user_id=user_id, **bot_args, get_logprobs=get_logprobs)
    ...
    if response is None: #if query_oracle just returned query_results bcos a function call was made, the last message in the messagesdict will be {"role": "user", "content": f"The following are the results of the function calls in JSON form: {query_results}. Use these results to answer the original query ('{user_input}') as if it is a natural conversation. Be concise. Do not use list, reply in paragraphs. Don't include specific addresses unless I ask for them. When you see units like miles, convert them to metrics, like kilometers."}
        for i in range(3):
            try:
                if CALC_PERPLEXITY and prompt != "" and prompt != ".":
                    print("PERPLEXITY MODE")
                    response = await toxicity_filter.generate_non_toxic_response(
                    ...
                    # response = await Bot.respond(messages[user_id].get(), user_id, **bot_args, get_logprobs=False) #messages is a global variable. Bot is ALWAYS MainAssistant, which is TemplateAssistant(user_info=user_info, assistant_settings=assistant_settings, assistant_descriptions=assistant_description)
                """
                ...
                if SELFCHECKGPT and prompt != "" and prompt != ".":
                    samples = []
    ...
    try:
        ...
        if HALLUCINATION_TESTING and prompt != "" and prompt != ".":
            ...
            if CALC_PERPLEXITY:
                ...
                perplexity = np.exp(1/num_tokens * -np.mean(logprobs))
        ...
        for exercise in unfinished_exercises:
            ...
            for question in questions:
                ...
                while not valid_response:
                    ...
                    # await websocket.send_text("{END_TURN}") # needed for testing over websocket e.g. feeding prompts, generating simulated conversations
...
def check_qa(user_query, embeddings, questions, answers, user_assistants):
    ...
    if answer:
        ...
        match_info = {
            "prompt": user_query,
            "question": match["most_relevant_Q"],
            "answer": match["most_relevant_A"],
        }
...
{"Exercise 1": "Getting to know your child.", "Exercise 2.1": "Connecting compassionately with your happy child.", "Exercise 2.2": "Connecting compassionately with your sad child."}"""
    ...
    assistant_message = await streaming_response(websocket, MainAssistant, user_id, prompt="", user_info=user_info, query_results=res, total_tokens_used_today=total_tokens_used_today)
    ...
    try:
        while True:
            if not exercises_completed or feedback_collected: #NEED TO DO ERROR CHECK BCOS IF U REFRESH PAGE, IT COULD BE THAT FEEDBACK HAS ALREADY BEEN COLLECTED BUT FEEDBACK_COLLECTED WILL BE SET TO FALSE AGAIN --> perhaps store in db, or have a class session_info 
                # wait for user input
                # await websocket.send_text("{END_TURN}")
                ...
                assistant_message = await streaming_response(websocket, MainAssistant, user_id, prompt=user_input, user_info=user_info, query_results={}, total_tokens_used_today=total_tokens_used_today)

                # log scenario match
                print(scenario_exercises)
                if scenario_exercises:
                    print("SCENARIO HAS BEEN FOUND")
                    filepath = "HALLUCINATION_scenario.json"

                    if os.path.exists(filepath):
                        with open(filepath, "r") as file:
                            scenario_matches = json.load(file)
                    else:
                        scenario_matches = []
                    ...
                    with open(filepath, "w") as file:
                        json.dump(scenario_matches, file, indent=4)


                # log q&a match
                if match is not None:
                    print("Q&A MATCH FOUND")
                    match["reply"] = assistant_message

                    # log question/answer match
                    filepath = "HALLUCINATION_QNA.json"

                    if os.path.exists(filepath):
                        with open(filepath, "r") as file:
                            qna_matches = json.load(file)
                    else:
                        qna_matches = []

                    qna_matches.append(match)

                    with open(filepath, "w") as file:
                        json.dump(qna_matches, file, indent=4)
